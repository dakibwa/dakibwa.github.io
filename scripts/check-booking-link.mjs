...
if (body.paymentMode === "postpay") {
  if (body.stripeReady !== true) {
    console.error("After-lesson charging is on but the Worker reports that Stripe is not ready.");
    process.exit(1);
  }
  ...
  if (!stripePublishableMode || (expectedStripeMode && stripePublishableMode !== expectedStripeMode)) {
    console.error(
      `After-lesson charging is on but NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY is ${stripePublishableMode || "missing/invalid"}; ` +
